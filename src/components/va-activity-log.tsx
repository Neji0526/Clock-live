...
// A row is treated as having a screenshot if the tracker captured one within
// this window of the event. The desktop/extension recorder snaps periodically,
// so we surface the nearest frame rather than one-per-event.
const SHOT_MATCH_WINDOW_MS = 6 * 60_000;

/** Index of the entry in `sortedTs` (ascending) closest to `target`, or -1. */
function nearestIndex(sortedTs: number[], target: number): number {
  const n = sortedTs.length;
  if (n === 0) return -1;
  if (target <= sortedTs[0]) return 0;
  if (target >= sortedTs[n - 1]) return n - 1;
  let lo = 0, hi = n - 1;
  while (lo <= hi) {
    const mid = (lo + hi) >> 1;
    if (sortedTs[mid] === target) return mid;
    if (sortedTs[mid] < target) lo = mid + 1; else hi = mid - 1;
  }
  // lo = first index > target, hi = last index < target
  return (target - sortedTs[hi]) <= (sortedTs[lo] - target) ? hi : lo;
}
...
export function VaActivityLog({ vaId }: { vaId: string }) {
  ...
  // Screenshot preview: either an exact `path` (a click's own frame) or a
  // `reel` of the day's shots opened at the frame nearest the clicked event.
  const [zoom, setZoom] = useState<{ path?: string; shots?: Shot[]; index?: number } | null>(null);
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      const [acts, steps, sess, shots] = await Promise.all([
      ...
      return {
        ...
        screenshots: (shots.data ?? []) as Shot[],
      };
    },
  });
  ...
  // Day's screenshots, ascending — used both as the lightbox reel and to match
  // each activity row to the frame captured nearest that moment.
  const dayShots = useMemo<Shot[]>(() => q.data?.screenshots ?? [], [q.data]);
  const shotTimes = useMemo(() => dayShots.map((s) => new Date(s.captured_at).getTime()), [dayShots]);
  const shotIdxFor = useCallback(
    (ts: number) => {
      const i = nearestIndex(shotTimes, ts);
      if (i < 0) return -1;
      return Math.abs(shotTimes[i] - ts) <= SHOT_MATCH_WINDOW_MS ? i : -1;
    },
    [shotTimes],
  );
                  ...
                  <ol className="space-y-1">
                    {g.items.map(it => (
                      <FeedRow
                        key={it.key}
                        item={it}
                        shotIdx={shotIdxFor(it.ts)}
                        onOpenShot={(p) => setZoom({ path: p })}
                        onOpenReel={(i) => setZoom({ shots: dayShots, index: i })}
                      />
                    ))}
                  </ol>
                </div>
...
function FeedRow({
  ...
  item: FeedItem;
  /** Index into the day's screenshot reel nearest this row, or -1 if none. */
  shotIdx: number;
  onOpenShot: (p: string) => void;
  onOpenReel: (index: number) => void;
}) {
  const time = new Date(item.ts).toLocaleTimeString([], { hour: "numeric", minute: "2-digit", second: "2-digit" });
  const hasNearby = shotIdx >= 0;
  if (item.kind === "visit") {
    const isNativeApp = !item.url; // desktop agent activity rows don't carry a URL
    const Icon = isNativeApp ? AppWindow : Globe;
    return (
      <li
        className={`grid grid-cols-[68px_20px_1fr_auto] items-start gap-2 text-sm py-1 px-1 rounded ${hasNearby ? "hover:bg-muted/60 cursor-zoom-in" : "hover:bg-muted/40"}`}
        onClick={hasNearby ? () => onOpenReel(shotIdx) : undefined}
        role={hasNearby ? "button" : undefined}
        title={hasNearby ? "View the screenshot captured around this time" : undefined}
      >
        ...
        <span className="flex items-center gap-1.5 pt-0.5">
          {hasNearby && <ImageIcon className="size-3.5 text-gold/70 shrink-0" />}
          <span className="text-[11px] tabular-nums text-muted-foreground">{fmtDuration(item.durationSec)}</span>
        </span>
      </li>
    );
  }
  // Clicks carry their own exact frame; fall back to the nearest reel frame.
  const exactPath = item.screenshotPath;
  const clickable = !!exactPath || hasNearby;
  return (
    <li
      ...
      onClick={clickable ? () => { if (exactPath) onOpenShot(exactPath); else onOpenReel(shotIdx); } : undefined}
