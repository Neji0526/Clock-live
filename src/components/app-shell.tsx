...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const renderNavLink = (n: NavItem) => {
    ...
    return (
      <Link
        key={n.to}
        to={n.to}
        className={`group relative flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm transition-all duration-200 ${
          active
            ? "bg-gradient-to-r from-emerald-600 to-emerald-700 text-white font-medium shadow-sm shadow-emerald-900/25"
            : "text-sidebar-foreground/70 hover:bg-sidebar-accent/60 hover:text-sidebar-foreground hover:translate-x-0.5"
        }`}
      >
        <Icon className={`size-4 transition-colors ${active ? "text-white" : ""}`} /> {n.label}
      </Link>
    );
  };
  ...
  const SidebarBody = (
    <>
      <div className="flex items-center gap-2.5 px-1 py-1">
        <ClockMark size={30} className="text-primary shrink-0" />
        <div className="font-display text-2xl text-primary leading-none">ClockWork</div>
      </div>
      ...
      <button
        ...
        className="mt-5 group flex items-center gap-2 w-full rounded-lg bg-gradient-to-r from-emerald-600 to-emerald-700 px-3 py-2.5 text-xs font-semibold text-white shadow-sm shadow-emerald-900/25 hover:from-emerald-600 hover:to-emerald-600 transition-colors press"
      >
        <Zap className="size-3.5 shrink-0" />
        <span className="flex-1 text-left">Quick actions</span>
        <kbd className="hidden sm:inline-flex items-center gap-0.5 rounded border border-white/15 bg-black/25 px-1.5 py-0.5 text-[10px] font-medium text-white/90">
          {isMac ? "⌘" : "Ctrl"} K
        </kbd>
      </button>
      ...
      <div className="mt-auto pt-4 border-t border-sidebar-border space-y-2">
        <div className="flex items-center gap-2 rounded-xl border border-sidebar-border bg-sidebar-accent/40 px-2.5 py-2">
          <Link
            to="/settings"
            className="flex flex-1 min-w-0 items-center gap-2.5 rounded-lg transition-colors"
          >
            <span className="grid place-items-center size-9 shrink-0 rounded-full bg-sidebar-foreground text-sidebar text-xs font-semibold">
              {initials}
            </span>
            <span className="min-w-0 leading-tight">
              <span className="block truncate text-sm font-semibold text-sidebar-foreground">{profile?.display_name ?? "—"}</span>
              <span className="mt-1 inline-flex items-center rounded-full border border-gold/30 bg-gold/10 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-[0.15em] text-gold">
                {profile?.role}
              </span>
            </span>
          </Link>
  ...
  return (
    <div className="ambient-backdrop min-h-screen flex bg-background">
      ...
      <aside className="sidebar-rail hidden md:flex w-64 flex-col shrink-0 sticky top-0 h-screen self-start overflow-y-auto border-r border-sidebar-border bg-gradient-to-b from-sidebar to-[oklch(0.14_0.012_160)] p-5">
        {SidebarBody}
      </aside>
        ...
        <div className="md:hidden flex items-center justify-between surface-glass px-4 py-3 sticky top-0 z-20">
          <div className="flex items-center gap-2.5">
            <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
              ...
              <SheetContent side="left" className="sidebar-rail w-72 p-5 flex flex-col bg-sidebar">
                {SidebarBody}
              </SheetContent>
            </Sheet>
